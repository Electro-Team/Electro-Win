...
namespace Electro.UI.ViewModels.DNS
{
    public class DNSViewModel : BaseModel
    {
        ...
        private String getHardwareIp()
        {
            ...
            var content = new FromU
            return id+ mtherBoard+"___PC";
        }
        ...
        */
        ...
        private async void OutputHandler(object sendingProcess, DataReceivedEventArgs outLine)
        {
            ...
            if (output != null)
            {
                if (output.Contains("Initialization Sequence Completed"))
                {
                    ...
                    await client.GetStringAsync("http://127.0.0.1:5000/newConnection/" + deviceId);
                    IsTurnedOn = true;
        ...
        private async void Dialer_DialCompleted(object sender, DialCompletedEventArgs e)
        {
            ...
            if (PingHost("172.16.0.1"))
            {
                psi.Arguments = "/C \"" + AppContext.BaseDirectory + "//route12.bat\" 172.16.0.1";
            } else if (PingHost("172.16.4.1"))
            {
                psi.Arguments = "/C \"" + AppContext.BaseDirectory + "//route12.bat\" 172.16.4.1";
            } else
            {
                IsTurnedOn = false;
                serviceText = "● Disconnected";
                UnsetDNS();
                return;
            }
            */
